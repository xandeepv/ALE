...
{
	//printf("The GetFileName for %s%s%s started. \n", folder, name, extension);
	char *fileName = new char[strlen(folder) + strlen(name) + strlen(extension) + 1];
	//fileName11 = new char[strlen(folder) + strlen(name) + strlen(extension) + 1];
	sprintf(fileName, "%s%s%s", folder, name, extension);
	//printf("The GetFileName for %s completed. \n", fileName);
	return(fileName);
}
