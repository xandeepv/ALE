...
{
	int i,j;
	printf("Training for Potential with FeatCnt %d and PotCnt %d \n", features.GetCount(), potentials.GetCount());
	for (i = 0; i < features.GetCount(); i++) {
		printf("getting Features for Image %s \n", typeid(*features[i]).name());
		//if (typeid(*features[i]).name() == "LColourSiftFeature") {
		//	j = i;
		//	continue;
		//}
		features[i]->LoadTraining();
	}
	for (i = 0; i < potentials.GetCount(); i++) {
		printf("getting Potentials for Image %d \n", i);
		//if (i == j) continue;
