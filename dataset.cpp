...
LMsrcDataset::LMsrcDataset() : LDataset()
{
	...
	imageExtension = ".bmp";  // msrc is .bmp
	groundTruthFolder = "Msrc/GroundTruth/";
	groundTruthExtension = ".bmp";  // msrc is .bmp
	trainFolder = "Msrc/Result/Train/";
	testFolder = "Msrc/Result/Crf/";

	clusterPointsPerKDTreeCluster = 20;
	clusterKMeansMaxChange = 0.01;
...
void LMsrcDataset::Init()
{
	...
	if(f != NULL)
	{
		...
		fclose(f);
		printf("Train File Number = from Train file list %d \n", trainImageFiles.GetCount(), trainFileList);
	}
	...
	if(f != NULL)
	{
		...
		fclose(f);
		printf("Test File Number = %d from testfile list = %s\n", testImageFiles.GetCount(), testFileList);
		printf("All Img File Number = %d \n", allImageFiles.GetCount());
	}
}
