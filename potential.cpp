...
void LDenseUnaryPixelPotential::Train(LList<char *> &trainImageFiles)
{
	...
	printf("Train Image files count %d with features size %d \n", fileSum, features.GetCount());

	InitTrainData(trainImageFiles);
	printf("IninTrainImg completed. \n");
	int totalFeatures = 0;
	for(i = 0; i < features.GetCount(); i++) totalFeatures += buckets[i];
	printf("Buckets added with totalFeat completed. \n");
	integralBuckets = new int[features.GetCount()];
	integralBuckets[0] = 0;
	for(i = 1; i < features.GetCount(); i++) integralBuckets[i] = integralBuckets[i - 1] + buckets[i - 1];
	printf("Buckets added with IntegralBuckets completed. \n");
	...
	printf("IntegralPoints completed. \n");
	int processors = 1;
#ifdef MULTITHREAD
...
void LDenseUnaryPixelPotential::InitTrainData(LList<char *> &trainImageFiles)
{
	...
	printf("LdenseUnaryPixPot Buckets created with Feat.GetBucket() \n");
	...
	printf("LdenseUnaryPixPot all vars created \n");
	for(i = 0; i < fileSum; i++)
	{
		char *fileName;
	...
	printf("LdenseUnaryPixPot Sliding Windows created \n");
	...
	printf("LdenseUnaryPixPot Class Prob created \n");
	...
	for(i = 0; i < fileSum; i++)
	{
		...
		printf("LdenseUnaryPixPot get Label Set done. \n");
		unsigned int labelBits = 0;
		for(k = 0; k < classNo; k++) if(thisOccurence[k]) labelBits |= (1 << k);
		printf("LdenseUnaryPixPot labelbits done. \n");
		int width, height;

		char *fileName;
		printf("File name is %s \n", trainImageFiles[i]);
		fileName = GetFileName(dataset->imageFolder, trainImageFiles[i], dataset->imageExtension);
		//if (i==0) fileName = "Msrc/Images/frame0412.bmp ";
		//else fileName = "Msrc/Images/frame0385.bmp ";
		printf("LdenseUnaryPixPot get i-th Imagefilename = %s. \n",fileName);
		LRgbImage rgbImage(fileName);
		//LGreyImage rgbImage(fileName);
		printf("LdenseUnaryPixPot Image read to Rgb \n");
		delete[] fileName;

		LLabImage labImage(rgbImage);
		printf("LdenseUnaryPixPot Image read to lab. \n");
		CalculateIntegralImages(labImage, &integralImages[i], subSample, &width, &height, trainImageFiles[i]);
		printf("LdenseUnaryPixPot Img read and Intergals calc done \n");
		int subWidth = (width + subSample - 1) / subSample;
        int subHeight = (height + subSample - 1) / subSample;
		...
		fileName = GetFileName(dataset->groundTruthFolder, trainImageFiles[i], dataset->groundTruthExtension);
		//if (i == 0) fileName = "Msrc/GroundTruth/frame0412.bmp ";
		//else fileName = "Msrc/GroundTruth/frame0385.bmp ";
		LLabelImage groundTruth(fileName, domain);
		delete[] fileName;
	...
	printf("LdenseUnaryPixPot All pixel values created \n");
	...
	printf("LdenseUnaryPixPot Label each bit done. \n");
	...
	printf("LdenseUnaryPixPot Init completed \n");
}
...
void LStatsUnarySegmentPotential::Train(LList<char *> &trainImageFiles)
{
	...
	numExamples = 0;
	printf("[Potential.cpp->Train] Feature counts = %d \n", features.GetCount());
	buckets = new int[features.GetCount()];
	for(i = 0; i < features.GetCount(); i++) buckets[i] = features[i]->GetBuckets();
	...
	int *imageSamples;
	printf("[Potential.cpp->Train] Train Img counts = %d \n", trainImageFiles.GetCount());
	...
	{
		char *fileName;
		printf("[Potential.cpp->Train] Train Img %s Layers counts = %d \n", trainImageFiles[m], layers.GetCount());
		for(i = 0; i < layers.GetCount(); i++) layers[i]->Initialize(trainImageFiles[m], 0);

		dataset->GetLabelSet(thisOccurence, trainImageFiles[m]);
		printf("[Potential.cpp->Train] Train Img %s got LabelSet \n", trainImageFiles[m]);
		unsigned int labelBits = 0;
		for(i = 0; i < classNo; i++) if(thisOccurence[i]) labelBits |= (1 << i);

		double *classPix;
		classPix = new double[classNo];
		printf("[Potential.cpp->Train] Train Img %s segmented?  = %d \n", trainImageFiles[m], dataset->Segmented(trainImageFiles[m]));
	...
	delete[] thisOccurence;
	printf("[Potential.cpp->Train] Train Img %s added feature layers. \n", trainImageFiles[m]);
	threshold = total * maxClassRatio;
	double *classProbs = new double[classNo];
	for(i = 0; i < classNo; i++) if(classCounts[i] != 0) classProbs[i] = threshold / (double) classCounts[i];
	...
	printf("[Potential.cpp->Train] Train Img %s added wts,featVals,ClassProbs \n", trainImageFiles[m]);
	printf("[Potential.cpp->Train]  Reading data..\n");
	...
	printf("[Potential.cpp->Train] Train Img %s completed SegCounts and bucketcounts \n", trainImageFiles[m]);
	...
	printf("[Potential.cpp->Train] Train Img %s Train Complete \n", trainImageFiles[m]);
}
